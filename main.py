...
def create_dataframe():
    table = run_query()
    df = pd.json_normalize(table.data)
...
def create_sf_object(model_df):
    sf = StatsForecast(models=[CrostonOptimized()],freq='MS',n_jobs=-1) # MS is month start
    sf.fit(df=model_df)
    return sf
...
def make_predictions(ids, horizon):
    ...
    predicted_df = sf.predict(h=horizon)
    return predicted_df.to_csv(header=True)
...
if __name__ == "__main__":
    ...
    with st.expander("Forecast"):
        if len(product_ids) == 0:
            ...
            if forecast_btn:
                csv_string = make_predictions(product_ids, horizon)
                st.download_button(label="Download Forecast", data=csv_string, file_name="forecast.csv", mime="text/csv")
